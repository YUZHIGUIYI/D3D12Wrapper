...
namespace gfxshim
{
	...
	// Helper functions
	REFIID ID3D12CommandQueueWrapper::GetRiid() const
	{
		return m_riid;
	}

	IUnknown* ID3D12CommandQueueWrapper::GetWrappedObject()
	{
		return m_object;
	}

	const IUnknown* ID3D12CommandQueueWrapper::GetWrappedObject() const
	{
		return m_object;
	}

	uint32_t ID3D12CommandQueueWrapper::GetRefCount() const
	{
		return m_ref_count.load(std::memory_order_seq_cst);
	}

	// IUnknown
	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::QueryInterface(REFIID riid, void** object)
	{
		// TODO: check wrap
		...
		if (IsEqualIID(riid, IID_Queue_11_On_12))
		{
			D3D12_WRAPPER_DEBUG("Invoke ID3D12CommandQueueWrapper::QueryInterface, equal riid");
			return S_OK;
		}
		encode::WrapObject(riid, object);
		D3D12_WRAPPER_DEBUG("Invoke ID3D12CommandQueueWrapper::QueryInterface, get wrapped object");
		return result;
	}

	ULONG STDMETHODCALLTYPE ID3D12CommandQueueWrapper::AddRef()
	{
		const auto result = ++m_ref_count;
		return result;
	}

	ULONG STDMETHODCALLTYPE ID3D12CommandQueueWrapper::Release()
	{
		const auto result = --m_ref_count;
		return result;
	}
	...
	// ID3D12Object
	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::GetPrivateData(const GUID &guid, UINT *pDataSize, void *pData)
	{
		const auto result = GetWrappedObjectAs<ID3D12Object>()->GetPrivateData(guid, pDataSize, pData);
		return result;
	}

	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::SetPrivateData(const GUID &guid, UINT DataSize, const void *pData)
	{
		const auto result = GetWrappedObjectAs<ID3D12Object>()->SetPrivateData(guid, DataSize, pData);
		return result;
	}

	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::SetPrivateDataInterface(const GUID &guid, const IUnknown *pData)
	{
		const auto result = GetWrappedObjectAs<ID3D12Object>()->SetPrivateDataInterface(guid, encode::GetWrappedObject<IUnknown>(pData));
		return result;
	}

	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::SetName(LPCWSTR Name)
	{
		return GetWrappedObjectAs<ID3D12Object>()->SetName(Name);
	}

	// ID3D12DeviceChild
	HRESULT STDMETHODCALLTYPE ID3D12CommandQueueWrapper::GetDevice(const IID &riid, void **ppvDevice)
	{

...
namespace gfxshim
{
	struct ID3D12GraphicsCommandList10Wrapper final : ID3D12GraphicsCommandList10
	{
		private:
		...
		inline static std::unordered_map<void *, ID3D12GraphicsCommandList10Wrapper *> s_d3d12_graphics_command_list_manager{};
		inline static std::mutex s_d3d12_graphics_command_list_mutex{};

	public:
		ID3D12GraphicsCommandList10Wrapper(REFIID riid, IUnknown *object);

		virtual ~ID3D12GraphicsCommandList10Wrapper() = default;

		// ID3D12GraphicsCommandList10Wrapper manager
		static void InsertD3D12GraphicsCommandList(void *object_key, ID3D12GraphicsCommandList10Wrapper *graphics_command_list)
		{
			std::lock_guard guard{ s_d3d12_graphics_command_list_mutex };
			s_d3d12_graphics_command_list_manager[object_key] = graphics_command_list;
		}

		static ID3D12GraphicsCommandList10Wrapper *QueryExistingD3D12GraphicsCommandList(void *object_key)
		{
			std::lock_guard guard{ s_d3d12_graphics_command_list_mutex };
			if (s_d3d12_graphics_command_list_manager.contains(object_key))
			{
				return s_d3d12_graphics_command_list_manager[object_key];
			}
			return nullptr;
		}
		...
		void STDMETHODCALLTYPE SetPipelineState1(
				ID3D12StateObject* pStateObject) override;

		void STDMETHODCALLTYPE DispatchRays(
				const D3D12_DISPATCH_RAYS_DESC* pDesc) override;
		...
		void STDMETHODCALLTYPE RSSetShadingRateImage(
				ID3D12Resource* shadingRateImage) override;

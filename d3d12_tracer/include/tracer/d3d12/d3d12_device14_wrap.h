...
namespace gfxshim
{
	struct ID3D12Device14Wrapper final : ID3D12Device14
	{
	private:
		...
		inline static std::unordered_map<void *, ID3D12Device14Wrapper *> s_d3d12_device_manager{};
		inline static std::mutex s_d3d12_device_mutex{};

	public:
		ID3D12Device14Wrapper(REFIID riid, IUnknown *object);

		virtual ~ID3D12Device14Wrapper() = default;

		// ID3D12Device12Wrapper manager
		static void InsertD3D12Device(void *object_key, ID3D12Device14Wrapper *device)
		{
			std::lock_guard guard{ s_d3d12_device_mutex };
			s_d3d12_device_manager[object_key] = device;
		}

		static ID3D12Device14Wrapper *QueryExistingD3D12Device(void *object_key)
		{
			std::lock_guard guard{ s_d3d12_device_mutex };
			if (s_d3d12_device_manager.contains(object_key))
			{
				return s_d3d12_device_manager[object_key];
			}
			return nullptr;
		}
